...
enum TokenKind {
    ...
    CATCH("catch"), CHAR("char"), CLASS("class"), CONTINUE("continue"),
    DEFLT("default"), DO("do"), DOUBLE("double"), ELSE("else"),
    EXTENDS("extends"), FINALLY("finally"), FOR("for"), IF("if"),
    IMPLEMENTS("implements"), IMPORT("import"), INSTANCEOF("instanceof"),
    INT("int"), INTERFACE ("interface"), LONG("long"), NEW("new"),
    PACKAGE("package"), PRIVATE("private"), PROTECTED("protected"),
    PUBLIC("public"), RETURN("return"), STATIC("static"), SUPER("super"),
    SWITCH("switch"),  THIS("this"), THROW("throw"),THROWS("throws"),
    TRY("try"), VOID("void"), WHILE("while"),

    // Operators.
    ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"), LAND("&&"), LE("<="), LNOT("!"),
    MINUS("-"), PLUS("+"), PLUS_ASSIGN("+="), STAR("*"), DIV("/"), REM("%"), NOT("~"), OR("|"),
    XOR("^"), AND("&"), ALSHIFT("<<"), ARSHIFT(">>"), LRSHIFT(">>>"), QUESTION("?"), COLON(":"),
    NOT_EQUAL("!="), DIV_ASSIGN("/="), MINUS_ASSIGN("-="), STAR_ASSIGN("*="), REM_ASSIGN("%="), ARSHIFT_ASSIGN(">>="),
    LRSHIFT_ASSIGN(">>>="), GE(">="), ALSHIFT_ASSIGN("<<="), LT("<"), XOR_ASSIGN("^="), OR_ASSIGN("|="), LOR("||"),
    AND_ASSIGN("&="),
    ...
    STRING_LITERAL("<STRING_LITERAL>"), TRUE("true"), LONG_LITERAL("<LONG_LITERAL>"), DOUBLE_LITERAL("<DOUBLE_LITERAL>");
